...
- ✅ A.1.1 Проверить наличие собранного JAR файла
  ...
  - **Результат:** JAR файл найден (77 МБ, дата сборки: 2025-12-05)
  
- ✅ A.1.2 Проверить наличие frontend сборки
  - Путь: `code/frontend/dist/spa/`
  - Проверить наличие файлов
  - **Результат:** Frontend сборка найдена (index.html и assets/)
...
- ✅ A.1.4 Создать/проверить директорию для данных TDB2
  ...
  - **Результат:** Директория существует
...
- ✅ A.2.1 Запустить приложение
  ...
  - Вариант 2: Вручную:
    ...
    java -Dorg.springframework.boot.logging.LoggingSystem=none \
         ...
         --server.port=8083
    ```
  - **Результат:** Приложение запущено вручную (PID: 1204), процесс работает на порту 8083
  
- ✅ A.2.2 Проверить запуск
  - Health check: `curl http://localhost:8083/health`
  - Ожидаемый результат: `{"application":"FEONT","status":"UP"}`
  - **Результат:** ✅ Health check успешен: `{"application":"FEONT","status":"UP"}`
  - Приложение доступно на http://localhost:8083
...
- ✅ A.3.1 Открыть приложение в браузере
  ...
  - **Результат:** Frontend загружается корректно, ошибок в консоли нет
...
- ✅ A.3.2 Проверить навигацию
  ...
  - **Результат:** Все страницы доступны и работают корректно
...
- ✅ A.3.3 Проверить SPARQL endpoints
  ...
  - **Результат:** Все endpoints доступны и работают

#### A.4 Ввод тестовых данных через веб-интерфейс

- A.4.1 Подготовить тестовые SPARQL UPDATE запросы

**Запрос 1: Добавление классов в онтологию** ✅ Выполнено

**Примечание:** Используется префикс `https://feont.ontoline.ru/ontology/`, который уже настроен в проекте.

**Результат:** 
- Запрос выполнен успешно через веб-интерфейс (тип запроса: UPDATE)
- Данные добавлены в граф `<urn:ontology>`
- Проверка через SELECT запрос подтвердила наличие классов: `feont:Department`, `feont:Employee` и свойств: `feont:name`, `feont:worksIn`
- Визуализация на GraphPage работает корректно
